(() => {
  ...
  function fallbackSlides(){
    return [
      {title:'Royal Farm Machinery',subtitle:'Verified tractors, harvesters and implements from trusted sellers.',cta_text:'Enter Marketplace',cta_route:'market',image_url:'./harvester-logo-full.jpg'},
      {title:'Fresh Spare Parts',subtitle:'Newly added belts, bearings, blades, filters and workshop-ready stock.',cta_text:'View Latest',cta_route:'market',image_url:'./harvester-logo-full.jpg'},
      {title:'Sponsored Farm Deals',subtitle:'Boosted sellers receive premium visibility across the mandi.',cta_text:'Seller Plans',cta_route:'membership',image_url:'./harvester-logo-full.jpg'}
    ];
  }
  ...
  function home(){
    ...
    return `<section class="hero video-hero"><video class="hero-bg-video" autoplay muted loop playsinline preload="metadata" poster="./harvester-logo-full.jpg"><source src="./hero-bg.mp4" type="video/mp4"></video><div class="hero-copy"><span class="eyebrow">Royal agricultural marketplace</span><h1>Premium farm machinery, implements and spare parts marketplace.</h1><p>Harvester Parts brings farmers, dealers and workshops together in a trusted agri mandi for machines, parts, secure orders and verified sellers.</p><div class="hero-actions"><button class="primary" data-route="market">Enter Marketplace</button><button class="ghost" data-route="sell">Become a Seller</button><button class="ghost" data-route="membership">Seller Plans</button></div></div><div class="hero-card glass"><img src="https://images.unsplash.com/photo-1592982537447-7440770cbfc9?auto=format&fit=crop&w=1200&q=80" alt="Agriculture"><div class="stats"><div class="stat"><b data-count="${state.stats.products||state.products.length}">0</b><span>Live products</span></div><div class="stat"><b data-count="${categoryCount}">0</b><span>Categories</span></div><div class="stat"><b data-count="${sellerCount}">0</b><span>Verified sellers</span></div><div class="stat"><b data-count="${state.stats.orders||0}">0</b><span>Orders</span></div></div></div></section>${homeCarousel()}${agriFactCard()}<section><div class="section-head"><h2>Shop by crop and machine need</h2><p class="muted">Browse agricultural categories built for farmers, dealers and workshops.</p></div><div class="agri-category-grid compact">${cats.slice(0,8).map(categoryCard).join('')}</div></section><section><div class="section-head"><h2>Fresh in the agri mandi</h2><button class="ghost" data-route="market">View all</button></div><div class="grid">${liveProducts.slice(0,6).map(productCard).join('')||empty('No live products yet.')}</div></section>`;
  }
