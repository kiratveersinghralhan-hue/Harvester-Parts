(() => {
  ...
  const {products,categories,brands,states,languages,plans,titles} = window.HP_DATA;
  const cfg = window.HP_CONFIG;
  let state = { route:'home', lang:localStorage.hp_lang || '', cart: JSON.parse(localStorage.hp_cart||'[]'), user: JSON.parse(localStorage.hp_user||'null'), filter:{q:'',category:'',brand:'',state:'',type:''}, selectedProduct:null };
  ...
  function filtered(){return products.filter(p=>(!state.filter.q || (p.name+p.brand+p.category).toLowerCase().includes(state.filter.q.toLowerCase())) && (!state.filter.category || p.category===state.filter.category) && (!state.filter.brand || p.brand===state.filter.brand) && (!state.filter.state || p.state===state.filter.state) && (!state.filter.type || p.type===state.filter.type));}
  function productPage(id){ const p=products.find(x=>x.id===id)||products[0]; $('#app').innerHTML=`<section class="product-page"><div><div class="gallery-main"><img src="${p.image}" alt="${p.name}"></div><section class="section"><p class="eyebrow">Related Products</p><div class="carousel">${products.filter(x=>x.category===p.category&&x.id!==p.id).slice(0,8).map(productCard).join('')}</div></section></div><aside class="detail-panel"><span class="chip">${p.condition}</span> <span class="chip verified">Verified Seller</span><h1>${p.name}</h1><p>${p.desc}</p><div class="price">${money(p.price)}</div><p><b>Brand:</b> ${p.brand}<br><b>Model:</b> ${p.model}<br><b>Location:</b> ${p.district}, ${p.state}<br><b>Seller:</b> ${p.seller}</p><button class="btn full" id="addCart">${p.type==='part'?'Add to Cart':'Send Enquiry'}</button><button class="btn dark full" style="margin-top:10px" id="whatsappBtn">WhatsApp Seller</button><button class="ghost full" style="margin-top:10px" data-route="chat">Open Chat</button></aside></section>`; $('#addCart').onclick=()=>addCart(p); $('#whatsappBtn').onclick=()=>open(`https://wa.me/${cfg.WHATSAPP_NUMBER}?text=${encodeURIComponent('I am interested in '+p.name+' on Harvester Parts')}`,'_blank'); }
  ...
  function afterRender(r){
    ...
    ['q','fType','fCat','fBrand','fState'].forEach(id=>{$('#'+id)?.addEventListener('input',e=>{const map={q:'q',fType:'type',fCat:'category',fBrand:'brand',fState:'state'}; state.filter[map[id]]=e.target.value; render();});});
    $('#clearFilters')?.addEventListener('click',()=>{state.filter={q:'',category:'',brand:'',state:'',type:''};render();});
