(() => {
  ...
  const state = { user:null, profile:null, seller:null, products:[], reviews:[], publicProfiles:{}, cart:[], wishlist:[], siteSlides:[], messages:[], unreadMessages:0, factIndex:0, route:'home', currentProduct:null, lang:localStorage.hp_lang || 'en', stats:{products:0,categories:0,sellers:0,orders:0}, admin:{orders:[],sellers:[],products:[],reports:[],contacts:[],plans:[],boosts:[],users:[],badges:[],events:[],memberships:[],reviews:[],docUrls:{},balances:[],payoutAccounts:[],payoutRequests:[],ledger:[],siteSlides:[],notifications:[],analytics:[]}, finance:{balance:null,payoutAccount:null,payoutRequests:[],ledger:[]}, realtimeReady:false };
  const SITE_ORIGIN = 'https://harvesterparts.in';
  const VALID_ROUTES = new Set(['home','market','product','seller','cart','checkout','login','account','sell','messages','orders','admin','membership','rewards','categories','about','contact','how','support','request-part','legal','terms','privacy','refund','shipping','razorpay','seller-policy','buyer-policy','payout-policy','fees-policy','prohibited-policy','dispute-policy','grievance']);
  function normalizeRouteName(name){ const r=String(name||'home').trim().toLowerCase().replace(/\.html$/,''); return ({plans:'membership',plan:'membership','seller-plans':'membership',badges:'rewards',rewards:'rewards',order:'orders',message:'messages',parts:'market','buy-parts':'market',browse:'market','request':'request-part','request-part':'request-part','request-a-part':'request-part',termsconditions:'terms','terms-and-conditions':'terms','privacy-policy':'privacy','refund-policy':'refund','refund-cancellation':'refund','cancellation-policy':'refund','shipping-policy':'shipping','delivery-policy':'shipping','payment-policy':'razorpay','razorpay-payment-policy':'razorpay','seller-payout-policy':'payout-policy','payout':'payout-policy','fees':'fees-policy','fees-commission-policy':'fees-policy','commission':'fees-policy','prohibited':'prohibited-policy','prohibited-items-policy':'prohibited-policy','dispute-resolution-policy':'dispute-policy','disputes':'dispute-policy','grievance-redressal':'grievance','how-it-works':'how'}[r] || r); }
  ...
  function productUrlId(productOrId){
    const p=typeof productOrId==='object' ? productOrId : state.products.find(x=>String(x.id)===String(productOrId));
    if(!p) return encodeURIComponent(String(productOrId||''));
    return `${slugify([p.title,p.brand,p.model].filter(Boolean).join(' '))}-${p.id}`;
  }
  function findProductByRouteId(id){
    const raw=decodeURIComponent(String(id||''));
    return state.products.find(p=>String(p.id)===raw)
      || state.products.find(p=>productUrlId(p)===raw)
      || state.products.find(p=>raw.endsWith('-'+String(p.id)))
      || null;
  }
  function routeHref(name, params={}){
    name=normalizeRouteName(name);
    if(name==='home') return '/';
    if(name==='market') return params.category ? `/parts/${slugify(params.category)}` : (params.type==='machine'?'/machinery':'/parts');
    if(name==='categories') return '/categories';
    if(name==='product') return `/product/${productUrlId(params.product||params.id)}`;
    if(name==='seller') return `/seller/${encodeURIComponent(params.id||'')}`;
    if(name==='sell') return params.id ? `/sell/${encodeURIComponent(params.id)}` : '/sell';
    if(name==='messages') return params.id ? `/messages/${encodeURIComponent(params.id)}` : '/messages';
    const paths={membership:'/seller-plans',rewards:'/rewards',cart:'/cart',checkout:'/checkout',login:'/login',account:'/account',orders:'/orders',admin:'/admin',about:'/about',contact:'/contact',how:'/how-it-works',support:'/support','request-part':'/request-part',legal:'/legal',terms:'/terms-and-conditions',privacy:'/privacy-policy',refund:'/refund-cancellation-policy',shipping:'/shipping-delivery-policy',razorpay:'/razorpay-payment-policy','seller-policy':'/seller-policy','buyer-policy':'/buyer-policy','payout-policy':'/seller-payout-policy','fees-policy':'/fees-commission-policy','prohibited-policy':'/prohibited-items-policy','dispute-policy':'/dispute-resolution-policy',grievance:'/grievance-redressal'};
    return paths[name] || `/${name}`;
  }
  function navLink(routeName,label,cls='',params={}){
    const attrs=params.id?` data-id="${esc(params.id)}"`:'';
    return `<a class="${esc(cls)}" href="${esc(routeHref(routeName,params))}" data-route="${esc(routeName)}"${attrs}>${esc(label)}</a>`;
  }
  function categoryFromSlug(slug=''){
    const normalized=String(slug||'').toLowerCase();
    return AGRI_CATEGORIES.find(c=>slugify(c.title)===normalized)?.title || humanizeSlug(slug);
  }
  function profileInitials(name='HP'){
    return String(name||'HP').split(/[\s@.]+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase() || 'HP';
  }
  ...
  function avatarMarkup(profile={},name='Seller',className=''){
    ...
    return `<span class="profile-photo ${esc(className)}">${url?`<img src="${esc(url)}" alt="${esc(name)}" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><i style="display:none">${esc(initials)}</i>`:`<i>${esc(initials)}</i>`}</span>`;
  }
  function reviewStats(sellerId){
    const rows=(state.reviews||[]).filter(r=>String(r.seller_id)===String(sellerId) && String(r.status||'approved')!=='hidden');
  ...
  function ratingStars(value=0){
    ...
    return `<span class="rating-stars" aria-label="${Number(value||0).toFixed(1)} out of 5">${[1,2,3,4,5].map(n=>`<i class="${n<=rounded?'filled':''}">★</i>`).join('')}</span>`;
  }
  ...
  function userPoints(user=state.profile, seller=state.seller, products=state.products){
    ...
    let pts=approved*120 + pending*25;
    if(user?.profile_completed) pts+=50;
    if(seller?.status==='approved') pts+=180;
    pts=Math.max(pts,Number(user?.points||0));
  ...
  function applyLang(){
    ...
    const routeLabels = {
      home:'Home', market:'Buy Parts', sell:'Sell', membership:'Seller Plans', rewards:'Rewards', categories:'Categories', how:'How it works',
      about:'About Us', contact:'Contact Us', support:'Support', 'request-part':'Request a Part', legal:'Legal Centre', terms:'Terms', privacy:'Privacy', refund:'Refunds', shipping:'Shipping', razorpay:'Payments', 'payout-policy':'Payouts', grievance:'Grievance', messages:'Messages', account:'My Account', cart:'Cart', checkout:'Checkout', orders:'Order History', admin:'Admin Panel'
    };
    ...
    $$('.side-menu button[data-route], .side-menu a[data-route], .bottom-nav button, .bottom-nav a[data-route], .nav-tabs button, .nav-tabs a[data-route]').forEach(el=>{
  ...
  async function purchaseMembership(key){
    ...
    const activate=async(paymentId)=>{
      ...
      await sendAdminNotice('membership_purchase','New seller plan purchase', `${state.profile?.full_name||state.user.email||'Seller'} purchased ${plan.name} for ${money(plan.price)}`, {...purchase,email:state.user.email||'',phone:state.profile?.phone||''});
  ...
  function showSuccessPopup(title='Changes completed',message='Your updates are now live across Harvester Parts.'){
    let modal=$('#successPopup');
    if(!modal){
      modal=document.createElement('div'); modal.id='successPopup'; modal.className='success-popup';
      modal.innerHTML='<div class="success-popup-card"><span class="success-check">✓</span><h2></h2><p></p><button class="primary" type="button">Done</button></div>';
    ...
    clearTimeout(window.__hpSuccessPopupTimer);
    window.__hpSuccessPopupTimer=setTimeout(()=>modal.classList.remove('show'),3600);
  }
  ...
  function clearMarketSessionFilters(){
    ['hp_market_category','hp_market_brand','hp_market_model','hp_market_type','hp_market_query','hp_market_condition'].forEach(k=>sessionStorage.removeItem(k));
  }
  function route(name, params={}){
    ...
    if(name==='admin' && !isAdminUser()){
      closeMenu();
      toast(state.user?'Admin access only':'Login required for admin access');
      name=state.user?'home':'login';
      params={};
    }
    closeMenu();
    if(name==='market') clearMarketSessionFilters();
    if(params.category){ sessionStorage.hp_market_category=params.category; }
    if(params.brand){ sessionStorage.hp_market_brand=params.brand; }
    if(params.model){ sessionStorage.hp_market_model=params.model; }
    if(params.type){ sessionStorage.hp_market_type=params.type; }
    state.route=name;
    state.currentProduct=params.id||null;
    const nextPath = routeHref(name,params);
    if(location.pathname + location.search !== nextPath) history.pushState({route:name,id:params.id||''},'',nextPath);
    render();
    recordInstallEngagement();
    setTimeout(()=>window.scrollTo({top:0,behavior:'smooth'}),30);
  }
  function parseRoute(){
    const h=location.hash.replace('#','');
    if(h) return h.split('/').map(decodeURIComponent);
    const parts=decodeURIComponent(location.pathname.replace(/\/+$/,'') || '/').split('/').filter(Boolean);
    if(!parts.length) return ['home'];
    const first=normalizeRouteName(parts[0]);
    const qs=new URLSearchParams(location.search);
    if(parts[0]==='product') return ['product',parts.slice(1).join('/')];
    if(parts[0]==='seller') return ['seller',parts[1]||''];
    if(parts[0]==='messages') return ['messages',parts[1]||''];
    if(parts[0]==='sell') return ['sell',parts[1]||''];
    if(parts[0]==='parts' && parts[1]){ clearMarketSessionFilters(); sessionStorage.hp_market_category=categoryFromSlug(parts[1]); return ['market']; }
    if(parts[0]==='brand'){
      clearMarketSessionFilters();
      sessionStorage.hp_market_brand=humanizeSlug(parts[1]||'');
      sessionStorage.hp_market_model=parts[2]?humanizeSlug(parts.slice(2).join('-')):'';
      return ['market'];
    }
    if(parts[0]==='machinery'){ clearMarketSessionFilters(); sessionStorage.hp_market_type='machine'; return ['market']; }
    if(parts[0]==='used-machinery'){ clearMarketSessionFilters(); sessionStorage.hp_market_type='machine'; sessionStorage.hp_market_condition='used'; return ['market']; }
    if(parts[0]==='parts' || parts[0]==='marketplace' || parts[0]==='buy-parts'){ clearMarketSessionFilters(); return ['market']; }
  ...
  function mediaMarkup(src,alt='Product media',cls=''){
    const safe=esc(src||'');
    if(isVideoUrl(src)) return `<video class="${esc(cls)}" src="${safe}" muted playsinline controls preload="metadata" poster="${placeholder()}"></video>`;
  ...
  async function init(){
    if(localStorage.hp_launch_clean_version !== LAUNCH_CLEAN_VERSION){
      ['hp_products','hp_cart','hp_wishlist','hp_last_order','hp_checkout_coupon','hp_fact_ts','hp_fact_index','hp_sellFormDraft'].forEach(k=>localStorage.removeItem(k));
    ...
    sessionStorage.removeItem('hp_sellFormDraft');
    localStorage.hp_visit_count = String((Number(localStorage.hp_visit_count||0)||0)+1);
    ...
    bindShell(); applyLang(); await loadSession(); await loadProducts(); await loadSellerReviews(); await loadSiteContent(); await loadFinanceData(); await loadMessages(); loadCart(); loadWishlist(); syncMenu(); render(); setupScroll(); setupFinanceRealtime(); setupProfileRealtime(); setupAnalytics();
  ...
  function scheduleInstallPrompt(delay=700){
    ...
    const visits=Number(localStorage.hp_visit_count||0);
    const engagement=Number(sessionStorage.hp_install_engagement||0);
    if(visits<2 && engagement<2) return;
  ...
  function recordInstallEngagement(){
    sessionStorage.hp_install_engagement=String((Number(sessionStorage.hp_install_engagement||0)||0)+1);
    scheduleInstallPrompt(1600);
  }
  function bindShell(){
    ...
    document.addEventListener('click', e=>{
      ...
      if(routeEl){
        ...
        if(routeEl.dataset.category) params.category=routeEl.dataset.category;
        if(routeEl.dataset.brand) params.brand=routeEl.dataset.brand;
        if(routeEl.dataset.model) params.model=routeEl.dataset.model;
        if(routeEl.dataset.type) params.type=routeEl.dataset.type;
        route(routeEl.dataset.route, params);
        return;
      }
    ...
    const routeFromLocation=()=>{ const [r,id]=parseRoute(); state.route=r||'home'; state.currentProduct=id||null; closeMenu(); render(); };
  ...
  function setupPageMotion(){
    const targets=$$('#app > section,#app .page-card,#app .product-card,#app .admin-fold,#app .admin-console-fold').filter((el,i,arr)=>!arr.some(other=>other!==el&&other.contains(el)&&other.classList.contains('product-card')));
    if(matchMedia('(prefers-reduced-motion: reduce)').matches||!('IntersectionObserver' in window)){
      targets.forEach(el=>el.classList.add('motion-visible')); return;
    }
    window.__hpMotionObserver?.disconnect();
    window.__hpMotionObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{
      entry.target.classList.toggle('motion-visible',entry.isIntersecting);
    }),{threshold:.06,rootMargin:'40px 0px -7% 0px'});
    targets.forEach((el,index)=>{
      el.classList.add('motion-reveal'); el.style.setProperty('--motion-delay',`${Math.min(index%5,4)*34}ms`);
      const rect=el.getBoundingClientRect(); if(rect.top<innerHeight*.96&&rect.bottom>0) el.classList.add('motion-visible');
      window.__hpMotionObserver.observe(el);
    });
  }
  ...
  function syncMenu(updateLang=true){
    ...
    const menuAvatar=$('#sideMenu .menu-head img'); if(menuAvatar){ menuAvatar.src=state.profile?.avatar_url||'/logo-192.png'; menuAvatar.alt=state.profile?.full_name||'Harvester Parts'; menuAvatar.onerror=()=>{menuAvatar.src='/logo-192.png';}; }
  ...
  async function loadProducts(){
    if(sb){
      ...
      const profileCols='*, sellers(business_name,status), users(email,full_name,badge_title,active_membership,membership_key,membership_title,avatar_url,headline,bio,city,state,business_type,experience_years,profile_completed,created_at,role,points)';
      const buildQuery=(cols)=>{
        let query=sb.from('products').select(cols);
        if(isAdminUser()) return query.order('status',{ascending:true}).order('is_boosted',{ascending:false}).order('created_at',{ascending:false});
        const visibleUid = state.user?.id || '00000000-0000-0000-0000-000000000000';
        return query.or(`status.eq.approved,user_id.eq.${visibleUid}`).order('is_boosted',{ascending:false}).order('created_at',{ascending:false});
      };
      let {data,error}=await buildQuery(profileCols);
      if(error && /avatar_url|headline|bio|business_type|experience_years/i.test(String(error.message||''))){
    ...
    const liveLocal=state.products.filter(isVisibleProduct);
    state.stats.products=liveLocal.length;
    state.stats.categories=[...new Set(liveLocal.map(p=>p.category).filter(Boolean))].length;
    state.stats.sellers=[...new Set(liveLocal.map(p=>p.user_id||p.seller_id).filter(Boolean))].length;
  }
  async function loadSellerReviews(){
    if(sb){
      try{
        const {data,error}=await sb.from('seller_reviews').select('*').order('created_at',{ascending:false}).limit(600);
        if(!error&&data){ state.reviews=data; return; }
      }catch(e){}
    }
    try{ state.reviews=JSON.parse(localStorage.hp_seller_reviews||'[]'); }catch(e){ state.reviews=[]; }
  }
  async function loadPublicProfile(userId){
    const id=String(userId||'');
    if(!id||state.publicProfiles[id]||window.__hpPublicProfileLoading===id) return;
    window.__hpPublicProfileLoading=id;
    let profile=(state.products||[]).find(p=>String(p.user_id)===id)?.users || null;
    if(sb){
      try{
        const {data,error}=await sb.from('public_seller_profiles').select('*').eq('auth_id',id).maybeSingle();
        if(!error&&data) profile=data;
      }catch(e){}
    }
    if(profile) state.publicProfiles[id]=profile;
    window.__hpPublicProfileLoading='';
    if(state.route==='seller'&&String(state.currentProduct||'')===id) render();
  }
  function setupProfileRealtime(){
    if(!sb||window.__hpProfileRealtime) return;
    try{
      window.__hpProfileRealtime=sb.channel('public-profile-sync').on('postgres_changes',{event:'UPDATE',schema:'public',table:'users'},payload=>{
        const updated=payload.new||{}; const id=String(updated.auth_id||updated.user_id||''); if(!id) return;
        state.publicProfiles[id]={...(state.publicProfiles[id]||{}),...updated};
        state.products.forEach(product=>{ if(String(product.user_id)===id) product.users={...(product.users||{}),...updated}; });
        if(!shouldHoldRender()) render();
      }).subscribe();
    }catch(e){}
  }
  ...
  function productCard(p){
    ...
    return `<article class="product-card fade-up"><div class="product-img"><a href="${esc(routeHref('product',{product:p}))}" data-route="product" data-id="${esc(productUrlId(p))}"><img src="${productImage(p)}" onerror="this.src='${placeholder(p.category)}'" alt="${p.title||'Product'}"></a></div><div class="product-body"><div>${boosted} <span class="badge verified">Verified Farm Listing</span></div><h3><a href="${esc(routeHref('product',{product:p}))}" data-route="product" data-id="${esc(productUrlId(p))}">${p.title||'Agricultural Listing'}</a></h3><p class="muted">${p.category||'Spare Part'} • ${p.brand||'Harvester Parts'}</p><div class="price-row"><span class="price">${money(p.price)}</span><span class="chip">Confirmed before order</span></div><div class="actions"><a class="ghost" href="${esc(routeHref('product',{product:p}))}" data-route="product" data-id="${esc(productUrlId(p))}">Details</a><button class="secondary" onclick="HP.addToCart('${p.id}')">Add Cart</button><button class="primary wide" onclick="HP.buyNow('${p.id}')">Buy Now</button></div></div></article>`;
  }
  function productCardNew(p){
    const boosted=p.is_boosted?'<span class="badge owner royal-seal">Sponsored</span>':'';
    const unit=productSaleUnit(p);
    const href=routeHref('product',{product:p});
    const rid=productUrlId(p);
    return `<article class="product-card fade-up"><div class="product-img"><a href="${esc(href)}" data-route="product" data-id="${esc(rid)}">${productMedia(p)}</a></div><div class="product-body"><div>${boosted} <span class="badge verified">Verified Farm Listing</span></div><h3><a href="${esc(href)}" data-route="product" data-id="${esc(rid)}">${esc(p.title||'Agricultural Listing')}</a></h3><p class="muted">${esc(p.category||'Spare Part')} • ${esc(p.brand||'Harvester Parts')}</p><div class="price-row"><span class="price">${money(p.price)} <small>/ ${esc(unit)}</small></span><span class="chip">Confirmed before order</span></div><div class="actions"><a class="ghost" href="${esc(href)}" data-route="product" data-id="${esc(rid)}">Details</a><button class="secondary" onclick="HP.addToCart('${esc(p.id)}')">Add Cart</button><button class="primary wide" onclick="HP.buyNow('${esc(p.id)}')">Buy Now</button></div></div></article>`;
  }
  ...
  function categoryCard(c){
    const count = visibleProducts().filter(p=> c.filters.some(f=>String(p.category||'').toLowerCase().includes(f.toLowerCase()) || String(p.title||'').toLowerCase().includes(f.toLowerCase()))).length;
    return `<a class="agri-category-card pro-cat fade-up" href="${esc(routeHref('market',{category:c.title}))}" data-route="market" data-category="${esc(c.title)}"><div class="cat-mark"><span class="cat-emoji" aria-hidden="true">${categoryIconSvg(c)}</span></div><div><small>${esc(c.group)}</small><h3>${esc(c.title)}</h3><p>${esc(c.desc)}</p><span>${count} active listings</span></div></a>`;
  }
  ...
  function featureCarousel(){
    ...
    return `<section class="home-carousel-section"><div class="section-head"><h2>Marketplace highlights</h2><p class="muted">Quick rotating cards for buying, selling, plans and verified listings.</p></div><div class="home-carousel">${slides.map(s=>`<article class="carousel-slide"><img src="${esc(s.image_url||placeholder())}" onerror="this.src='${placeholder()}'" alt="${esc(s.title||'Marketplace update')}"><div><span class="eyebrow">Harvester Parts</span><h3>${esc(s.title||'Marketplace update')}</h3><p>${esc(s.subtitle||'Verified agriculture machinery and spare-part trading.')}</p><button class="primary" data-route="${esc(normalizeRouteName(s.cta_route||'market'))}">${esc(s.cta_text||'Open')}</button></div></article>`).join('')}</div></section>`;
  }
  ...
  function market(){
    ...
    const brand=sessionStorage.hp_market_brand||'';
    const model=sessionStorage.hp_market_model||'';
    const condition=sessionStorage.hp_market_condition||'';
    const search=String(new URLSearchParams(location.search).get('q') || sessionStorage.hp_market_query || '').trim();
    const matchesMarket=p=>{
      const hay=[p.title,p.category,p.brand,p.model,p.condition,p.description,p.city,p.district,p.state,p.pincode,p.part_number,p.oem_number,p.sku_code,p.compatible_machine].join(' ').toLowerCase();
      return matchesProductType(p,type)
        && (!selected || hay.includes(String(selected).toLowerCase()))
        && (!brand || String(p.brand||'').toLowerCase().includes(brand.toLowerCase()) || hay.includes(brand.toLowerCase()))
        && (!model || String(p.model||'').toLowerCase().includes(model.toLowerCase()) || hay.includes(model.toLowerCase()))
        && (!condition || String(p.condition||'').toLowerCase()===condition.toLowerCase())
        && (!search || hay.includes(search.toLowerCase()));
    };
    const shown=marketProducts.filter(matchesMarket);
    const contextLine=[selected&&`Category: ${selected}`,brand&&`Brand: ${brand}`,model&&`Model: ${model}`,condition&&`Condition: ${condition}`].filter(Boolean).join(' • ');
    return `<section class="page-card market-head-card hp92-market-head"><div><span class="eyebrow">Buyer marketplace</span><h1>Search verified farm machines and spare parts.</h1><p class="muted">Use fast suggestions, product type, category, condition and price filters to find the right farm item quickly.</p>${contextLine?`<div class="active-filter-strip"><span>${esc(contextLine)}</span><a href="${esc(routeHref('market'))}" data-route="market">Clear filters</a></div>`:''}</div><a class="primary" href="${esc(routeHref('sell'))}" data-route="sell">List Product</a><div class="market-tools"><select id="typeFilter"><option value="all" ${type==='all'?'selected':''}>All products</option><option value="machine" ${type==='machine'?'selected':''}>Machinery</option><option value="spare" ${type==='spare'?'selected':''}>Spare parts</option></select><div class="search-wrap"><input id="searchInput" autocomplete="off" value="${esc(search)}" placeholder="Search by part name, OEM number, machine or brand"><div id="searchSuggest" class="search-suggest"></div></div><select id="categoryFilter"><option value="">All categories</option>${categories.map(c=>`<option ${c===selected?'selected':''}>${esc(c)}</option>`).join('')}</select><select id="sortFilter"><option value="all">All listings</option><option value="condition_new">New</option><option value="condition_used">Used</option><option value="price_low">Price low to high</option><option value="price_high">Price high to low</option></select></div></section><section class="grid hp92-market-grid" id="marketGrid">${shown.map(productCardNew).join('')||empty('No live catalog. Ask sellers to list products.')}</section>`;
  }
  
  function productPage(id){
    const p=findProductByRouteId(id);
  ...
  function publicSellerPoints(uid,profile={},seller={},products=[]){
    const mine=(products||[]).filter(p=>String(p.user_id)===String(uid));
    const approved=mine.filter(p=>p.status==='approved').length;
    const pending=mine.filter(p=>p.status==='pending').length;
    let points=approved*120+pending*25+(profile.profile_completed?50:0)+(seller.status==='approved'?180:0);
    points=Math.max(points,Number(profile.points||0));
    if(profile.role==='admin'||String(profile.email||'').toLowerCase()===ADMIN_EMAIL) return 999999;
    return points;
  }
  function productPageV113(id){
    const p=findProductByRouteId(id);
    if(!p) return emptyPage('Product not found');
    const imgs=productImages(p); const profile=publicProfileFor(p.user_id,p); const sellerName=sellerDisplayName(p,profile);
    const loc=[p.city,p.district,p.state].filter(Boolean).join(', ') || [profile.city,profile.state].filter(Boolean).join(', ') || 'Location shared by seller';
    const ratings=reviewStats(p.user_id);
    const related=visibleProducts().filter(x=>String(x.id)!==String(p.id) && (String(x.category||'').toLowerCase()===String(p.category||'').toLowerCase() || String(x.sell_type||'')===String(p.sell_type||''))).slice(0,8);
    return `<section class="product-page"><div class="gallery page-card product-gallery"><div class="main-product-photo" onclick="HP.openGallery('${esc(p.id)}',0)">${mediaMarkup(imgs[0],p.title||'Product')}<span class="photo-count">${imgs.length} media${imgs.length>1?' files':''}</span></div>${imgs.length>1?`<div class="gallery-thumbs">${imgs.map((img,i)=>`<button onclick="HP.openGallery('${esc(p.id)}',${i})">${mediaMarkup(img,p.title||'Product')}</button>`).join('')}</div>`:''}</div><aside class="detail-stack sticky-buy"><div class="page-card"><span class="badge verified">Verified Farm Listing</span><h1>${esc(p.title||'Agricultural Product')}</h1><p class="muted">${esc(p.category||'Spare Part')} • ${esc(p.brand||'Harvester Parts')} ${p.model?`• ${esc(p.model)}`:''}</p><div class="price">${money(p.price)} <small>/ ${esc(productSaleUnit(p))}</small></div><p class="muted">Confirm final price, freight, dimensions and availability before payment.</p><div class="seller-mini-card seller-mini-v113">${avatarMarkup(profile,sellerName,'seller-card-photo')}<div><b>${esc(sellerName)}</b><span>${ratings.count?`${ratings.average.toFixed(1)} rating • ${ratings.count} review${ratings.count===1?'':'s'}`:'New verified seller'}</span><small>${esc(loc)}</small></div><button class="ghost" onclick="HP.route('seller',{id:'${esc(p.user_id)}'})">Visit Profile</button></div><div class="actions"><button class="primary" onclick="HP.buyNow('${esc(p.id)}')">Buy Now</button><button class="secondary" onclick="HP.addToCart('${esc(p.id)}')">Add to Cart</button><button class="ghost" onclick="HP.toggleWishlist('${esc(p.id)}')">Wishlist</button><button class="ghost" onclick="HP.route('messages',{id:'${esc(p.id)}'})">Message Seller</button></div></div><div class="summary-card buyer-only-card"><h3>Buyer assurance</h3><div class="summary-row"><span>Seller</span><b>${esc(sellerName)}</b></div><div class="summary-row"><span>Rating</span><b>${ratings.count?ratings.average.toFixed(1)+' / 5':'New seller'}</b></div><div class="summary-row"><span>Payment</span><b>Secure checkout</b></div><div class="summary-row"><span>Delivery</span><b>${matchesProductType(p,'machine')?'Road / seller coordination':'Courier for spare parts'}</b></div></div></aside></section><section class="page-card product-detail-v113"><h2>Product details</h2><p>${esc(p.description||'Agricultural machinery or spare part listing from a verified seller. Confirm compatibility, condition, dimensions and delivery details before final purchase.')}</p><div class="stats"><div class="stat"><b>${esc(p.condition||'Stock')}</b><span>Condition</span></div><div class="stat"><b>${esc(p.weight_kg||'—')} kg</b><span>Weight</span></div><div class="stat"><b>${esc(p.state||'India')}</b><span>Location</span></div><div class="stat"><b>${esc(p.views||0)}</b><span>Views</span></div></div></section>${related.length?`<section class="product-carousel-section related-products"><div class="section-head"><h2>Similar products</h2><p class="muted">More verified listings related to this item.</p></div><div class="product-row-carousel">${related.map(productCardNew).join('')}</div></section>`:''}<div id="photoLightbox" class="photo-lightbox" aria-hidden="true"><button class="lightbox-close" onclick="HP.closeGallery()">×</button><button class="lightbox-nav prev" onclick="HP.stepGallery(-1)">‹</button><div id="lightboxMedia" class="lightbox-media">${mediaMarkup(imgs[0],p.title||'Product media')}</div><button class="lightbox-nav next" onclick="HP.stepGallery(1)">›</button><div id="lightboxCount" class="lightbox-count">1 / ${imgs.length}</div></div>`;
  }
  function productCompatibilityValues(p={}){
    const text=[p.compatible_machine,p.compatible_model,p.model,p.description].filter(Boolean).join('\n');
    const matches=[...String(text).matchAll(/(?:Compatible(?:\s+with)?|compatible machine|model(?:s)?):\s*([^\n]+)/ig)].flatMap(m=>String(m[1]||'').split(/[,;|]/));
    if(p.compatible_machine) matches.unshift(p.compatible_machine);
    if(p.model) matches.unshift(p.model);
    return [...new Set(matches.map(x=>String(x||'').trim()).filter(x=>x && !/not added/i.test(x)))].slice(0,8);
  }
  function productExtraPublicSections(p={}){
    const compat=productCompatibilityValues(p);
    const specs=[
      ['OEM / Part number', p.part_number || p.oem_number || detailValue(p.description,'OEM/part number')],
      ['Brand', p.brand],
      ['Model', p.model],
      ['Condition', p.condition],
      ['Weight', p.weight_kg?`${p.weight_kg} kg`:'' ],
      ['Location', [p.city,p.district,p.state,p.pincode].filter(Boolean).join(', ')],
      ['Sale unit', productSaleUnit(p)]
    ].filter(x=>String(x[1]||'').trim());
    return `${compat.length?`<section class="page-card compatible-card"><h2>Compatible With</h2><div class="compat-list">${compat.map(x=>`<span>${esc(x)}</span>`).join('')}</div><p class="muted">Compatibility is based only on information provided in this listing. Confirm fitment before purchase.</p></section>`:''}${specs.length?`<section class="page-card product-spec-summary"><h2>Listing specification</h2><div class="info-list">${specs.map(([k,v])=>`<div><span>${esc(k)}</span><b>${esc(v)}</b></div>`).join('')}</div></section>`:''}`;
  }
  function productPageProduction(id){
    const p=findProductByRouteId(id);
    const html=productPageV113(id);
    if(!p) return html;
    return html.replace('<div id="photoLightbox"', productExtraPublicSections(p) + '<div id="photoLightbox"');
  }
  function sellerReviewCards(sellerId){
    const rows=reviewStats(sellerId).rows.slice(0,20);
    if(!rows.length) return `<div class="seller-review-empty"><b>No reviews yet</b><span>Marketplace buyers can be the first to rate this seller.</span></div>`;
    return rows.map(review=>`<article class="seller-review-card"><div><b>${String(review.reviewer_id)===String(state.user?.id)?'You':'Marketplace buyer'}</b>${ratingStars(review.rating)}</div><p>${esc(review.comment||'Rating submitted without a written review.')}</p><small>${new Date(review.created_at||Date.now()).toLocaleDateString('en-IN')}</small></article>`).join('');
  }
  function sellerProfilePage(userId){
    const id=String(userId||''); const listings=visibleProducts().filter(p=>String(p.user_id)===id); const sample=listings[0]||state.products.find(p=>String(p.user_id)===id);
    const profile=publicProfileFor(id,sample); const seller=sample?.sellers || (id===String(state.user?.id)?state.seller:{}) || {};
    if(!sample&&!profile?.full_name) return `<section class="page-card"><h1>Seller profile</h1><p class="muted">Loading public seller details...</p></section>`;
    const name=profile.full_name || sample?.users?.full_name || seller.business_name || 'Verified seller';
    const ratings=reviewStats(id); const points=publicSellerPoints(id,profile,seller,state.products); const rank=rankForPoints(points);
    const location=[profile.city,profile.state].filter(Boolean).join(', ') || [sample?.city,sample?.state].filter(Boolean).join(', ') || 'India';
    const joined=profile.created_at?new Date(profile.created_at).toLocaleDateString('en-IN',{month:'short',year:'numeric'}):'Harvester Parts member';
    const isOwn=id===String(state.user?.id||'');
    const action=isOwn?`<button class="primary" data-route="account">Edit My Profile</button>`:(sample?`<button class="primary" onclick="HP.route('messages',{id:'${esc(sample.id)}'})">Message Seller</button>`:'');
    const reviewForm=!state.user?`<button class="secondary" data-route="login">Login to Rate Seller</button>`:isOwn?`<p class="muted">Reviews are written by other marketplace members.</p>`:`<form id="sellerReviewForm" class="seller-review-form"><input type="hidden" name="seller_id" value="${esc(id)}"><div class="rating-input" role="radiogroup" aria-label="Seller rating">${[5,4,3,2,1].map(n=>`<label><input type="radio" name="rating" value="${n}" required><span>${n} ★</span></label>`).join('')}</div><textarea name="comment" maxlength="400" placeholder="Share a useful review about communication, product accuracy or delivery"></textarea><button class="primary">Submit Rating</button></form>`;
    return `<section class="seller-storefront"><div class="seller-profile-hero page-card"><div class="seller-cover-band"></div><div class="seller-profile-top">${avatarMarkup(profile,name,'seller-profile-photo')}<div class="seller-profile-title"><span class="badge verified">Verified Seller</span><h1>${esc(name)}</h1><p>${esc(profile.headline||profile.business_type||'Agricultural machinery and spare parts seller')}</p><div class="seller-public-meta"><span>${esc(location)}</span><span>Member since ${esc(joined)}</span><span>${esc(rank.title)}</span></div></div><div class="seller-profile-actions">${action}</div></div><div class="seller-profile-stats"><div><b>${listings.length}</b><span>Live listings</span></div><div><b>${ratings.count?ratings.average.toFixed(1):'New'}</b><span>Seller rating</span></div><div><b>${ratings.count}</b><span>Reviews</span></div><div><b>${points>=999999?'MAX':points}</b><span>Trust points</span></div></div></div><div class="seller-store-layout"><section class="seller-store-main"><div class="seller-store-tabs"><span class="active">Listings</span><span>About</span><span>Reviews</span></div><div class="seller-listing-showcase"><div class="section-head compact"><div><h2>Seller listings</h2><p class="muted">Current products from this verified seller.</p></div></div><div class="grid seller-store-grid">${listings.map(productCardNew).join('')||empty('No live listings at the moment.')}</div></div><div class="page-card seller-reviews-section"><div class="section-head compact"><div><h2>Seller reviews</h2><p class="muted">Ratings help buyers understand seller communication and listing accuracy.</p></div><span class="badge">${ratings.count?ratings.average.toFixed(1)+' / 5':'New'}</span></div><div class="seller-review-list">${sellerReviewCards(id)}</div>${reviewForm}</div></section><aside class="seller-store-sidebar"><div class="page-card seller-about-card"><h2>About seller</h2><p>${esc(profile.bio||'This verified seller has not added a public introduction yet.')}</p><div class="info-list"><div><span>Profile type</span><b>${esc(profile.business_type||'Seller')}</b></div><div><span>Experience</span><b>${profile.experience_years?esc(profile.experience_years)+' years':'Not added'}</b></div><div><span>Location</span><b>${esc(location)}</b></div><div><span>Verification</span><b>Approved seller</b></div></div></div><div class="page-card seller-safety-card"><h3>Safe marketplace profile</h3><p>Private contact details, identity documents, bank information and pickup address are hidden.</p></div></aside></div></section>`;
  }
  async function submitSellerReview(form){
    if(!state.user) return route('login');
    const fd=new FormData(form); const sellerId=String(fd.get('seller_id')||''); const rating=Number(fd.get('rating')||0); const comment=String(fd.get('comment')||'').trim();
    if(!sellerId||sellerId===String(state.user.id)) return toast('You cannot rate your own seller profile.');
    if(rating<1||rating>5) return toast('Choose a rating from 1 to 5.');
    const payload={seller_id:sellerId,reviewer_id:state.user.id,rating,comment,status:'approved',updated_at:new Date().toISOString()};
    if(sb){
      const {error}=await sb.from('seller_reviews').upsert(payload,{onConflict:'seller_id,reviewer_id'});
      if(error) return toast('Review could not be saved. Run SUPABASE-PROFILES-REVIEWS.sql once, then try again.');
    } else {
      const old=(state.reviews||[]).find(r=>String(r.seller_id)===sellerId&&String(r.reviewer_id)===String(state.user.id));
      if(old) Object.assign(old,payload); else state.reviews.unshift({...payload,id:'local-review-'+Date.now(),created_at:new Date().toISOString()});
      localStorage.hp_seller_reviews=JSON.stringify(state.reviews);
    }
    await loadSellerReviews(); render(); showSuccessPopup('Rating submitted','Your seller review is now visible on this public profile.');
  }
  ...
  function ensureProductReportButton(){
    if(state.route!=='product' || !state.currentProduct) return;
    const product=findProductByRouteId(state.currentProduct);
    const reportId=product?.id || state.currentProduct;
    const actions=$('.product-page .actions');
    if(!actions || actions.querySelector('.report-btn')) return;
    actions.insertAdjacentHTML('beforeend', `<button class="ghost report-btn" onclick="HP.reportProduct('${esc(reportId)}')">Report listing</button>`);
  }
  function aboutPage(){
  ...
  function requestPartPage(){
    ...
    return `<section class="page-card request-part-hero"><span class="eyebrow">Request a Part</span><h1>Cannot find the spare part? Send your requirement.</h1><p class="muted">Share machine, brand, model, part number if known, photo and location. Admin can review it and sellers can be guided safely without exposing private contact details publicly.</p></section><section class="page-card request-part-card"><form id="partRequestForm" class="form request-part-form"><div class="sell-form-grid"><select name="machine_type" required><option value="">Machine type</option>${AGRI_CATEGORIES.filter(c=>c.group==='Machines').map(c=>`<option>${esc(c.title)}</option>`).join('')}<option>Other Machinery</option></select><input name="brand" placeholder="Brand, e.g. CLAAS, John Deere, Mahindra"><input name="model" placeholder="Machine model, e.g. Dominator 76"><input name="part_name" placeholder="Part name" required><input name="part_number" placeholder="OEM / part number if known"><select name="condition_preference"><option>Any condition</option><option>New only</option><option>Used acceptable</option><option>OEM preferred</option><option>Aftermarket acceptable</option></select><input name="location" placeholder="City, district, state" required></div><textarea name="description" placeholder="Describe the part, size, fitment, problem, urgency and quantity needed"></textarea><label class="file-label">Reference photo optional<input name="photo" type="file" accept="image/jpeg,image/png,image/webp,image/avif,image/heic,image/heif"></label><button class="primary">Submit Part Request</button></form></section>`;
  }
  async function submitPartRequest(form){
    ...
    if(!partName) return toast('Enter the part name you need.');
    if(!location) return toast('Enter your location.');
    let photo_url='';
    const file=fd.get('photo');
    if(file&&file.name){
      if(file.size>6*1024*1024) return toast('Reference photo must be under 6 MB.');
      if(sb){
        const safeName=file.name.replace(/[^a-z0-9.]/gi,'-');
        const path=`${state.user.id}/part-request-${Date.now()}-${safeName}`;
        const {error}=await sb.storage.from('product-images').upload(path,file,{upsert:true,contentType:file.type||undefined});
        if(!error){ const {data}=sb.storage.from('product-images').getPublicUrl(path); photo_url=data?.publicUrl||''; }
      } else photo_url=await fileToDataUrl(file);
    }
    const payload={user_id:state.user.id,buyer_email:state.user.email||'',buyer_name:state.profile?.full_name||'',machine_type:fd.get('machine_type')||'',brand:fd.get('brand')||'',model:fd.get('model')||'',part_name:partName,part_number:fd.get('part_number')||'',condition_preference:fd.get('condition_preference')||'Any condition',location,description:fd.get('description')||'',photo_url,status:'open',created_at:new Date().toISOString(),updated_at:new Date().toISOString()};
    let saved=false;
    if(sb){
      const {error}=await sb.from('part_requests').insert(payload);
      if(!error) saved=true;
      if(error){
        await sb.from('contact_messages').insert({user_id:state.user.id,name:payload.buyer_name||payload.buyer_email,topic:'Request a Part',message:`${payload.part_name} / ${payload.part_number}\n${payload.machine_type} ${payload.brand} ${payload.model}\n${payload.location}\n${payload.description}`}).catch(()=>{});
      }
    } else {
      const rows=JSON.parse(localStorage.hp_part_requests||'[]');
      rows.unshift({...payload,id:'local-pr-'+Date.now()});
      localStorage.hp_part_requests=JSON.stringify(rows.slice(0,100));
      saved=true;
    }
    await sendAdminNotice('part_request','New buyer part request', `${payload.buyer_name||payload.buyer_email||'Buyer'} needs ${payload.part_name}${payload.part_number?' / '+payload.part_number:''}`, payload);
    form.reset();
    showSuccessPopup('Part request sent', saved?'Your requirement is saved for marketplace review.':'Your request was sent through support because the part request table is not installed yet.');
  }
  ...
  function payoutAccountCard(){
    ...
    return `<div class="page-card payout-card"><div class="section-head compact"><h2>Payout method</h2><span class="badge">${esc(payoutMethodLabel(a))}</span></div><p class="muted">Choose one payout method. Only the required boxes for that method will be shown.</p><form id="payoutAccountForm" class="form payout-form"><select name="payout_method"><option value="upi" ${method==='upi'?'selected':''}>UPI</option><option value="bank" ${method==='bank'?'selected':''}>Bank account</option></select><div class="payout-upi-fields"><input name="upi_id" placeholder="UPI ID, e.g. name@upi" value="${esc(a.upi_id||'')}"></div><div class="payout-bank-fields"><input name="account_holder_name" placeholder="Account holder name" value="${esc(a.account_holder_name||state.profile?.full_name||'')}"><input name="bank_name" placeholder="Bank name" value="${esc(a.bank_name||'')}"><input name="account_number" placeholder="Account number" value="${esc(a.account_number||'')}"><input name="ifsc" placeholder="IFSC code" value="${esc(a.ifsc||'')}"></div><button class="primary">Save payout details</button></form></div>`;
  }
  async function savePayoutAccount(form){
    ...
    const payload={user_id:state.user.id,payout_method:method,account_holder_name:method==='bank'?fd.get('account_holder_name'):'',upi_id:method==='upi'?fd.get('upi_id'):'',bank_name:method==='bank'?fd.get('bank_name'):'',account_number:method==='bank'?fd.get('account_number'):'',ifsc:method==='bank'?String(fd.get('ifsc')||'').toUpperCase():'',updated_at:new Date().toISOString()};
  ...
  async function requestPayout(){
    ...
    await sendAdminNotice('payout_request','New seller payout request', `${state.profile?.full_name||state.user.email||'Seller'} requested payout of ${money(amount)}`, {...payload,email:state.user.email||'',phone:state.profile?.phone||''});
  ...
  async function placeOrder(form){
    ...
    await sendAdminNotice('new_order','New marketplace order', `Order ${orderId} placed for ${money(totals.total)} by ${fd.get('name')||state.user.email||'Buyer'}`, {order_id:orderId,buyer_id:state.user.id,buyer_email:state.user.email||'',buyer_name:fd.get('name'),buyer_phone:fd.get('phone'),amount:totals.total,shipping_amount:totals.shipping,platform_fee:totals.pf,payment_id:paidId||'manual_pending',items:itemRows});
  ...
  function bindFormDraft(formId,key){
    ...
    [...form.elements].forEach(el=>{
      ...
      if(el.closest('.hidden-fields')) return;
  ...
  function saveFormDraft(form,key){
    const data={};
    [...form.elements].forEach(el=>{ if(el.name && el.type!=='file' && el.type!=='password' && !el.disabled && !el.closest('.hidden-fields')){ if(el.type==='radio'){ if(el.checked) data[el.name]=el.value; } else data[el.name]=el.value; }});
  ...
  function accountPageV113(){
    if(!state.user) return loginPage();
    const isAdmin=isAdminUser();
    const profile=state.profile||{};
    const profileName=profile.full_name || state.user.user_metadata?.full_name || '';
    const email=state.user.email || 'Phone login account';
    const fullName=profileName || email;
    const phone=profile.phone || state.user.phone || '';
    const uid=profile.user_uid || ('HP-'+String(state.user.id||'account').replaceAll('-','').slice(0,8).toUpperCase());
    const pts=userPoints(); const rank=rankForPoints(pts); const memberPlan=activePlan();
    const role=isAdmin?'Platform Founder • 1 of 1':(memberPlan?.title || profile.membership_title || profile.badge_title || (state.seller?.status==='approved'?'Verified Seller':'Buyer / Seller'));
    const myProducts=state.products.filter(p=>String(p.user_id||'')===String(state.user.id));
    const approvedListings=myProducts.filter(p=>p.status==='approved').length;
    const pendingListings=myProducts.filter(p=>p.status!=='approved').length;
    const sellerStatus=state.seller?.status || (isAdmin?'approved':'not verified');
    const ratings=reviewStats(state.user.id);
    const publicProfileButton=(state.seller?.status==='approved'||isAdmin)?`<button class="secondary" onclick="HP.route('seller',{id:'${esc(state.user.id)}'})">View Public Profile</button>`:'';
    return `<section class="profile-page profile-page-v113">${userBanner()}<div class="profile-cover page-card ${isAdmin?'founder-profile-cover':''}">${avatarMarkup(profile,fullName,'profile-photo-large')}<div class="profile-main"><div class="profile-title-row"><div data-no-translate><h1>${esc(fullName)}</h1><p>${esc(profile.headline||'Agricultural marketplace member')}</p></div><span class="badge ${isAdmin?'owner':'verified'}">${esc(role)}</span></div><div class="profile-id"><span data-no-translate>${esc(uid)}</span> • <span>${esc(rank.title)}</span> • <span>${esc(sellerStatus)}</span></div><div class="profile-stats"><div><b>${myProducts.length}</b><span>Listings</span></div><div><b>${approvedListings}</b><span>Live</span></div><div><b>${ratings.count?ratings.average.toFixed(1):'New'}</b><span>Rating</span></div><div><b>${pts>=999999?'MAX':pts}</b><span>Points</span></div></div><div class="profile-actions"><button class="primary" data-route="sell">Sell an Item</button>${publicProfileButton}<button class="ghost" data-route="orders">My Orders</button>${isAdmin?'<button class="secondary" data-route="admin">Admin Panel</button>':''}</div></div></div><div class="profile-grid"><div class="page-card profile-edit-card profile-edit-v113"><div class="section-head compact"><div><h2>Edit profile</h2><p class="muted">Public seller information updates across your listings.</p></div><span class="badge">Private controls</span></div><form id="profileForm" class="form profile-form profile-form-v113"><div class="profile-photo-editor">${avatarMarkup(profile,fullName,'profile-photo-edit')}<label class="secondary avatar-upload-button">Change photo<input name="avatar" type="file" accept="image/jpeg,image/png,image/webp,image/avif,image/heic,image/heif"></label><small>Square JPG, PNG or WebP works best.</small></div><div class="profile-form-grid"><label><span>Full name</span><input name="full_name" value="${esc(profileName)}" placeholder="Full name" required autocomplete="name"></label><label><span>Phone number</span><input name="phone" type="tel" value="${esc(phone)}" placeholder="Phone number" autocomplete="tel"></label><label><span>Gender</span><select name="gender"><option value="">Prefer not to say</option><option value="Male" ${profile.gender==='Male'?'selected':''}>Male</option><option value="Female" ${profile.gender==='Female'?'selected':''}>Female</option><option value="Other" ${profile.gender==='Other'?'selected':''}>Other</option></select></label><label><span>Profile type</span><select name="business_type"><option value="">Select profile type</option>${['Farmer','Dealer','Agency','Workshop','Machine Owner','Parts Supplier','Other'].map(x=>`<option ${profile.business_type===x?'selected':''}>${x}</option>`).join('')}</select></label><label><span>Public city</span><input name="city" value="${esc(profile.city||state.seller?.city||'')}" placeholder="City / district"></label><label><span>Public state</span><select name="state">${stateOptions(profile.state||state.seller?.state||'')}</select></label><label><span>Experience</span><input name="experience_years" type="number" min="0" max="80" value="${esc(profile.experience_years||'')}" placeholder="Years in agriculture"></label><label class="wide"><span>Public headline</span><input name="headline" maxlength="90" value="${esc(profile.headline||'')}" placeholder="Example: Combine harvester parts dealer in Punjab"></label><label class="wide"><span>About you or your business</span><textarea name="bio" maxlength="500" placeholder="Describe your machinery, spare parts, experience and service area">${esc(profile.bio||'')}</textarea></label></div><div class="profile-privacy-note"><b>Public:</b> photo, name, headline, bio, city/state, rating, rank and listings. <b>Private:</b> email, phone and gender.</div><button class="primary">Save Profile Changes</button></form></div><div class="page-card profile-info-card"><h2>Private account overview</h2><div class="info-list"><div><span>User ID</span><b data-no-translate>${esc(uid)}</b></div><div><span>Email</span><b data-no-translate>${esc(email)}</b></div><div><span>Phone</span><b>${esc(phone||'Not added')}</b></div><div><span>Gender</span><b>${esc(profile.gender||'Not shared')}</b></div><div><span>Seller status</span><b>${esc(sellerStatus)}</b></div><div><span>Pending listings</span><b>${pendingListings}</b></div></div></div>${rankProgressCard()}${membershipMiniCard()}${accountBalanceCard()}${payoutAccountCard()}${badgeCollectionCard()}<div class="page-card profile-info-card"><h2>Quick tools</h2><div class="quick-grid"><button class="ghost" data-route="market">Enter Marketplace</button><button class="ghost" data-route="messages">Messages</button><button class="ghost" data-route="cart">Cart</button><button class="ghost" data-route="contact">Contact Support</button>${state.seller?.status==='approved'||isAdmin?'<button class="secondary" data-route="sell">Add New Listing</button>':'<button class="secondary" data-route="sell">Become Verified Seller</button>'}</div></div><div class="page-card profile-info-card"><h2>Trust and privacy</h2><p class="muted">Your public seller page never displays your email, phone number, gender, Aadhaar, bank details or pickup address.</p><div class="trust-pills"><span>Verified sellers</span><span>Admin review</span><span>Private contact details</span></div></div></div></section>`;
  }
  async function saveProfileV113(form){
    if(!state.user) return route('login');
    const fd=new FormData(form);
    const fullName=String(fd.get('full_name')||'').trim();
    if(fullName.length<2) return toast('Enter your full name.');
    let avatarUrl=state.profile?.avatar_url||'';
    const avatarFile=fd.get('avatar');
    if(avatarFile&&avatarFile.name){
      if(avatarFile.size>6*1024*1024) return toast('Profile photo must be under 6 MB.');
      if(sb){
        const safeName=avatarFile.name.replace(/[^a-z0-9.]/gi,'-');
        const path=`${state.user.id}/avatar-${Date.now()}-${safeName}`;
        const {error}=await sb.storage.from('profile-avatars').upload(path,avatarFile,{upsert:true,contentType:avatarFile.type||undefined});
        if(error) return toast('Profile photo upload failed. Run SUPABASE-PROFILES-REVIEWS.sql once, then try again.');
        const {data}=sb.storage.from('profile-avatars').getPublicUrl(path); avatarUrl=data?.publicUrl||avatarUrl;
      } else avatarUrl=await fileToDataUrl(avatarFile);
    }
    const patch={full_name:fullName,phone:String(fd.get('phone')||state.user.phone||'').trim(),gender:String(fd.get('gender')||''),avatar_url:avatarUrl,headline:String(fd.get('headline')||'').trim(),bio:String(fd.get('bio')||'').trim(),city:String(fd.get('city')||'').trim(),state:String(fd.get('state')||'').trim(),business_type:String(fd.get('business_type')||'').trim(),experience_years:Number(fd.get('experience_years')||0)||null,profile_completed:true,updated_at:new Date().toISOString()};
    if(sb){
      let {error}=await sb.from('users').update(patch).eq('auth_id',state.user.id);
      if(error&&/avatar_url|headline|bio|business_type|experience_years|updated_at/i.test(String(error.message||''))){
        const basic={full_name:patch.full_name,phone:patch.phone,gender:patch.gender,profile_completed:true};
        const fallback=await sb.from('users').update(basic).eq('auth_id',state.user.id); error=fallback.error;
        if(!error) toast('Basic profile saved. Run the profile upgrade SQL to enable photo and public seller details.');
      }
      if(error) return toast(error.message);
      try{ await sb.auth.updateUser({data:{full_name:patch.full_name,avatar_url:patch.avatar_url}}); }catch(e){}
    }
    state.profile={...(state.profile||{}),...patch};
    if(state.user.user_metadata) state.user.user_metadata={...state.user.user_metadata,full_name:patch.full_name,avatar_url:patch.avatar_url};
    state.publicProfiles[String(state.user.id)]={...state.profile};
    state.products.forEach(product=>{ if(String(product.user_id)===String(state.user.id)) product.users={...(product.users||{}),...patch}; });
    if(!sb) localStorage.hp_local_profile=JSON.stringify(state.profile);
    if(sb){ await loadSession(); await loadProducts(); }
    syncMenu(); render();
    showSuccessPopup('Profile updated','Your new name, photo and public seller details are now live across your listings.');
  }
  function sellerStatusCard(){
    ...
    return `<section class="seller-verify-page"><div class="page-card seller-verify-hero"><span class="eyebrow">${rejected?'Verification rejected':'Seller verification required'}</span><h1>${rejected?'Update your seller details.':'Become a verified Harvester Parts seller.'}</h1><p class="muted">${rejected?'Your previous request needs correction. Submit clear business, address and document details again.':'Submit one clean verification request before listing machinery or spare parts. This protects buyers and keeps the marketplace trustworthy.'}</p><div class="verify-step-grid"><div><b>1</b><span>Business details</span></div><div><b>2</b><span>Pickup address</span></div><div><b>3</b><span>Documents</span></div></div></div><div class="seller-verify-layout"><aside class="page-card verify-info-panel"><h2>What you need</h2><div class="verify-check-list"><span>Seller or business name</span><span>Phone number</span><span>State, district, city and pincode</span><span>Aadhaar front and back</span><span>Shop, stock or product photo</span></div><p class="muted">Only admin can view verification files. After approval, the listing tools will unlock.</p></aside>${sellerVerificationForm()}</div></section>`;
  }
  ...
  function productToSellDraft(p={}){
    ...
    if(p.part_number || p.oem_number) draft.part_number=p.part_number||p.oem_number;
    if(p.compatible_machine) draft.compatible_machine=p.compatible_machine;
    if(p.sku_code) draft.sku_code=p.sku_code;
    if(p.quantity_available) draft.listing_qty=p.quantity_available;
    if(p.sale_unit) draft.sale_unit=productSaleUnit(p);
    ...
    if(p.dimensions && typeof p.dimensions==='object'){
      Object.entries(p.dimensions).forEach(([label,value])=>{
        const specKey=specs.find(([name])=>name===label)?.[1];
        if(specKey && value) draft[specKey]=String(value).replace(/\s+(mm|cm|m|inch|ft|HP|rows|litre|kg|ton).*$/i,'');
      });
    }
  ...
  function sellPage(){
    ...
    const unitOptions=(selected='', units=['mm','cm','m','inch','ft'])=>{ const list=[...units]; if(selected && !list.includes(selected)) list.unshift(selected); return list.map(u=>`<option value="${u}" ${String(selected||'')===u?'selected':''}>${u}</option>`).join(''); };
    const specRow=(label,valueName,unitName,defaultUnit,note='',required=false,units=null)=>{
      const savedValue=d[valueName]||'';
      const selectedUnit=savedValue ? (d[unitName]||defaultUnit) : defaultUnit;
      const savedNote=savedValue ? (d[valueName+'_note']||'') : '';
      return `<tr><th>${esc(label)}${required?'<span class="required-dot">Required</span>':''}</th><td><input name="${valueName}" value="${esc(savedValue)}" placeholder="Enter value"></td><td><select name="${unitName}">${unitOptions(selectedUnit, units||['mm','cm','m','inch','ft'])}</select></td><td><input name="${valueName}_note" value="${esc(savedNote)}" placeholder="${esc(note||'Optional note')}"></td></tr>`;
    };
    const specTableHtml=(kind,rows,hint)=>`<div class="spec-table-card ${kind}-fields ${chosenType===kind?'':'hidden-fields'}"><div class="spec-table-head"><div><h3>${kind==='spare'?'Spare part dimensions':'Machinery dimensions'}</h3><p class="muted">${hint}</p></div><span>Use mm/cm where possible</span></div><div class="spec-table-wrap"><table class="listing-spec-table"><thead><tr><th>Specification</th><th>Value</th><th>Unit</th><th>Note</th></tr></thead><tbody>${rows.join('')}</tbody></table></div></div>`;
    const spareSpecRows=[
      specRow('Length / belt length','dim_length','dim_length_unit','mm','overall length or belt length'),
      specRow('Width','dim_width','dim_width_unit','mm','belt, blade or body width'),
      specRow('Height','dim_height','dim_height_unit','mm','overall height'),
      specRow('Outer diameter','dim_outer_dia','dim_outer_dia_unit','mm','bearing, bush, pulley or seal OD'),
      specRow('Inner diameter','dim_inner_dia','dim_inner_dia_unit','mm','bearing, bush or seal ID'),
      specRow('Thickness / pitch','dim_thickness','dim_thickness_unit','mm','thickness, pitch or tooth size')
    ];
    const machineSpecRows=[
      specRow('Overall length','dim_length','dim_length_unit','ft','transport length'),
      specRow('Overall width','dim_width','dim_width_unit','ft','transport width'),
      specRow('Overall height','dim_height','dim_height_unit','ft','transport height'),
      specRow('Working width','working_width','working_width_unit','ft','cutting, tillage or sowing width'),
      specRow('Capacity / HP','engine_hp','engine_hp_unit','HP','engine HP, rows, tank or load capacity',false,['HP','rows','litre','kg','ton']),
      specRow('Tyre / attachment size','attachment_size','attachment_size_unit','inch','tyre, blade, roller or attachment size')
    ];
  ...
  function listingSpecSummary(fd,type){
    const rows=type==='spare'
      ? [['Length / belt length','dim_length','dim_length_unit'],['Width','dim_width','dim_width_unit'],['Height','dim_height','dim_height_unit'],['Outer diameter','dim_outer_dia','dim_outer_dia_unit'],['Inner diameter','dim_inner_dia','dim_inner_dia_unit'],['Thickness / pitch','dim_thickness','dim_thickness_unit']]
      : [['Overall length','dim_length','dim_length_unit'],['Overall width','dim_width','dim_width_unit'],['Overall height','dim_height','dim_height_unit'],['Working width','working_width','working_width_unit'],['Capacity / HP','engine_hp','engine_hp_unit'],['Tyre / attachment size','attachment_size','attachment_size_unit']];
    return rows.map(([label,valueName,unitName])=>{
      const value=String(fd.get(valueName)||'').trim();
      if(!value) return '';
      const note=String(fd.get(valueName+'_note')||'').trim();
      return `${label}: ${value} ${fd.get(unitName)||''}${note?` (${note})`:''}`.trim();
    }).filter(Boolean);
  }
  ...
  async function submitAssistedListingRequest(form){
    ...
    if(itemType==='spare' && !String(fd.get('assist_part_number')||'').trim()) return toast('Enter OEM or part number for the spare part.');
  ...
  async function submitProduct(form){
    ...
    if(sellType==='spare' && !String(fd.get('compatible_machine')||'').trim()) return toast('Enter compatible machine or model.');
    ...
    if(editingProduct){
      ...
      if(sb){
        ...
        if(error && /seller_id|pincode|updated_at|part_number|oem_number|sku_code|compatible_machine|manufacturer|quantity_available|sale_unit|dimensions/i.test(String(error.message||''))){
          const fallback={...updatePayload};
          ['seller_id','pincode','updated_at','part_number','oem_number','sku_code','compatible_machine','manufacturer','quantity_available','sale_unit','dimensions'].forEach(k=>delete fallback[k]);
    ...
    if(sb){
      ...
      if(error && /seller_id|pincode|part_number|oem_number|sku_code|compatible_machine|manufacturer|quantity_available|sale_unit|dimensions/i.test(String(error.message||''))){ const fallback={...payload}; ['seller_id','pincode','part_number','oem_number','sku_code','compatible_machine','manufacturer','quantity_available','sale_unit','dimensions'].forEach(k=>delete fallback[k]); const res=await sb.from('products').insert(fallback); error=res.error; }
      if(error)return toast(error.message);
      await loadProducts();
    ...
    else await sendAdminNotice('new_listing','New product listing published', `${state.profile?.full_name||state.user.email||'Seller'} listed ${payload.title} for ${money(payload.price)}`, {...payload,seller_email:state.user.email||'',seller_phone:state.profile?.phone||''});
  ...
  function messagesPage(){
    ...
    const sellerName=p ? sellerDisplayName(p) : '';
  ...
  function messageProductSellerName(p){ return p ? sellerDisplayName(p) : 'Marketplace user'; }
  function messageThreads(){
    ...
    const routedProduct=state.route==='messages' && state.currentProduct ? messageProduct(state.currentProduct) : null;
  ...
  function selectedMessageThread(threads=[]){
    const routedProduct=state.route==='messages' ? String(state.currentProduct||'') : '';
  ...
  function selectMessageThread(key){
    ...
    if(location.pathname!=='/messages') history.replaceState(null,'','/messages');
    render();
  }
  ...
  async function sendAdminNotice(type, subject, body, payload={}){
    const notice={type,subject,body,payload,status:'unread',priority:'important',important:true,admin_email:ADMIN_ALERT_EMAIL,email_to:ADMIN_ALERT_EMAIL,created_at:new Date().toISOString(),source:'harvester-parts-website'};
    let delivered=false;
    const functionName=cfg.ADMIN_ALERT_FUNCTION || 'admin-alert';
    if(sb?.functions?.invoke){
      try{ const {error}=await sb.functions.invoke(functionName,{body:notice}); delivered=!error; }catch(e){}
    }
    if(!delivered && cfg.ADMIN_ALERT_WEBHOOK){
      try{
        const headers={'Content-Type':'application/json'};
        if(cfg.SUPABASE_ANON_KEY){ headers.apikey=cfg.SUPABASE_ANON_KEY; headers.Authorization=`Bearer ${cfg.SUPABASE_ANON_KEY}`; }
        const res=await fetch(cfg.ADMIN_ALERT_WEBHOOK,{method:'POST',headers,body:JSON.stringify(notice)});
        delivered=res.ok;
      }catch(e){}
    }
    if(!sb) return;
    try{ await sb.from('admin_notifications').insert({...notice,email_sent:delivered}); }catch(e){ try{ await sb.from('admin_notifications').insert({type,subject,body,payload,status:'unread',priority:'important',admin_email:ADMIN_EMAIL,email_sent:delivered}); }catch(_e){} }
  }
  async function reportProduct(id){
    if(!state.user) return route('login');
    const product=state.products.find(p=>String(p.id)===String(id));
    const reason=prompt('Tell admin what is wrong with this listing:','');
    if(reason===null) return;
    const clean=String(reason||'').trim();
    if(!clean) return toast('Add a short report reason');
    const payload={reporter_id:state.user.id,reporter_email:state.user.email||'',target_type:'product',target_id:id,product_id:id,product_title:product?.title||'',seller_id:product?.user_id||product?.seller_id||'',reason:clean,status:'open',created_at:new Date().toISOString()};
    if(sb){
      let {error}=await sb.from('reports').insert(payload);
      if(error && /product_id|reporter_email|seller_id|created_at/i.test(String(error.message||''))){
        const fallback={reporter_id:state.user.id,target_type:'product',target_id:id,reason:clean,status:'open'};
        const res=await sb.from('reports').insert(fallback); error=res.error;
      }
      if(error) return toast('Report could not be saved. Please use support page.');
    }
    await sendAdminNotice('product_report','Important product report', `${state.profile?.full_name||state.user.email||'User'} reported ${product?.title||'a listing'}: ${clean}`, payload);
    toast('Report sent to admin for review');
  }
  ...
  function adminReviewsPanel(){
    const rows=(state.admin.reviews?.length?state.admin.reviews:state.reviews)||[];
    const content=rows.slice(0,40).map(review=>`<div class="admin-review-row"><div><b>${Number(review.rating||0)} / 5 seller rating</b><p>${esc(review.comment||'No written comment.')}</p><small>Seller ${esc(String(review.seller_id||'').slice(0,12))} • ${new Date(review.created_at||Date.now()).toLocaleDateString('en-IN')}</small></div><span class="badge ${review.status==='hidden'?'danger-soft':'verified'}">${esc(review.status||'approved')}</span><div class="approval-actions"><button class="secondary" onclick="HP.setReviewStatus('${esc(review.id)}','approved')">Approve</button><button class="danger" onclick="HP.setReviewStatus('${esc(review.id)}','hidden')">Hide</button></div></div>`).join('');
    return `<div class="admin-panel admin-reviews-panel" id="adminReviewsPanel"><div class="section-head compact"><div><h2>Seller reviews</h2><p class="muted">Moderate ratings and remove abusive or irrelevant comments.</p></div><span class="badge">${rows.length}</span></div>${content||empty('No seller reviews yet.')}</div>`;
  }
  function adminPageV113(){
    if(!isAdminUser()) return emptyPage('Admin access only');
    const base=adminPage();
    const identityStart=base.indexOf('<section class="page-card admin-panel admin-identity-control">');
    const kpiStart=base.indexOf('<section class="admin-kpi-grid">');
    if(identityStart<0||kpiStart<0) return base;
    const pendingSellers=(state.admin.sellers||[]).filter(s=>['pending','provisional'].includes(s.status)).length;
    const openReports=(state.admin.reports||[]).filter(r=>r.status==='open').length;
    const assisted=(state.admin.notifications||[]).filter(n=>/assisted_listing|support_request|buyer_message/i.test(String(n.type||''))).length;
    const consoleHtml=`<section class="admin-command-center"><div class="admin-attention-strip"><div><small>Seller approvals</small><b data-attention="sellers">${pendingSellers}</b></div><div><small>Open reports</small><b data-attention="reports">${openReports}</b></div><div><small>Assisted listings</small><b data-attention="assisted">${assisted}</b></div><div><small>Reviews</small><b data-attention="reviews">${(state.admin.reviews||state.reviews||[]).length}</b></div></div><details class="admin-console-fold" open><summary><b>Attention and notifications</b><span>Open</span></summary><div>${adminNotificationsPanel()}</div></details><details class="admin-console-fold"><summary><b>Website analytics</b><span>Live</span></summary><div>${adminAnalyticsPanel()}</div></details><details class="admin-console-fold"><summary><b>Assisted listing requests</b><span>${assisted}</span></summary><div>${adminAssistedListingPanel()}</div></details><details class="admin-console-fold"><summary><b>Seller reviews</b><span>${(state.admin.reviews||state.reviews||[]).length}</span></summary><div>${adminReviewsPanel()}</div></details><details class="admin-console-fold"><summary><b>Founder identity and access</b><span>Settings</span></summary><div>${adminIdentityPanel()}</div></details></section>`;
    let output=base.slice(0,identityStart)+consoleHtml+base.slice(kpiStart);
    output=output.replace('>Plans</button></section>','>Plans</button><button onclick="document.getElementById(\'adminReviewsPanel\')?.scrollIntoView({behavior:\'smooth\',block:\'start\'})">Reviews</button></section>');
    return output;
  }
  async function setReviewStatus(id,status){
    if(!isAdminUser()) return toast('Admin access required');
    if(sb){ const {error}=await sb.from('seller_reviews').update({status,updated_at:new Date().toISOString()}).eq('id',id); if(error) return toast(error.message); }
    const review=(state.admin.reviews||state.reviews||[]).find(r=>String(r.id)===String(id)); if(review) review.status=status;
    await loadSellerReviews(); if(sb) await loadAdminProData(); render(); toast(`Review ${status}`);
  }
  ...
  async function loadAdminProData(){
    ...
    state.admin.reviews=await safe(()=>sb.from('seller_reviews').select('*').order('created_at',{ascending:false}).limit(300));
    if(state.admin.reviews.length) state.reviews=state.admin.reviews;
  ...
  function refreshAdminLists(){
    ...
    const reviews=$('#adminReviewsPanel'); if(reviews) reviews.outerHTML=adminReviewsPanel();
    const assisted=(state.admin.notifications||[]).filter(n=>/assisted_listing|support_request|buyer_message/i.test(String(n.type||''))).length;
    const attention={sellers:(state.admin.sellers||[]).filter(s=>['pending','provisional'].includes(s.status)).length,reports:(state.admin.reports||[]).filter(r=>r.status==='open').length,assisted,reviews:(state.admin.reviews||state.reviews||[]).length};
    Object.entries(attention).forEach(([key,value])=>{ const el=$(`[data-attention="${key}"]`); if(el) el.textContent=value; });
  }
  ...
  function legalFooter(){
    return `<footer class="company-footer royal-footer"><div class="footer-brand"><img src="/logo-192.png" alt="Harvester Parts"><div><b>Harvester Parts</b><span>India’s trusted agri mandi for farm machinery, implements and spare parts.</span></div></div><div class="footer-columns"><div><h4>Marketplace</h4>${navLink('market','Browse farm listings')}${navLink('request-part','Request a Part')}${navLink('sell','Sell machinery or parts')}${navLink('membership','Seller plans')}</div><div><h4>For farmers & dealers</h4>${navLink('categories','Categories')}${navLink('how','How it works')}${navLink('support','Support')}${navLink('contact','Contact')}</div><div><h4>Company</h4>${navLink('about','About Harvester Parts')}${navLink('terms','Terms')}${navLink('privacy','Privacy')}${navLink('refund','Refunds')}${navLink('shipping','Shipping')}${navLink('grievance','Grievance')}</div></div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Harvester Parts. All rights reserved.</span><span>Verified sellers • Secure checkout • Agricultural marketplace</span></div></footer>`;
  }
  function legalCardLink(key,label,desc){
    const doc=legalDocs[key]||{};
    return `<a class="legal-link-card" href="${esc(routeHref(key))}" data-route="${esc(key)}"><b>${esc(label||doc.title||key)}</b><span>${esc(desc||doc.subtitle||'Open policy')}</span></a>`;
  }
  ...
  function shortDescription(text='',fallback=''){
    return String(text||fallback||'').replace(/\s+/g,' ').trim().slice(0,155);
  }
  function absoluteMediaUrl(src=''){
    const value=String(src||'').trim();
    if(!value) return SITE_ORIGIN+'/logo-512.png';
    if(/^(https?:|data:|blob:)/i.test(value)) return value;
    return SITE_ORIGIN+'/'+value.replace(/^\.?\//,'');
  }
  function currentCanonicalPath(){
    if(state.route==='product'){
      const p=findProductByRouteId(state.currentProduct);
      return p ? routeHref('product',{product:p}) : '/product';
    }
    if(state.route==='market'){
      const brand=sessionStorage.hp_market_brand||'';
      const model=sessionStorage.hp_market_model||'';
      const cat=sessionStorage.hp_market_category||'';
      if(brand) return `/brand/${slugify(brand)}${model?'/'+slugify(model):''}`;
      if(cat) return routeHref('market',{category:cat});
      if(sessionStorage.hp_market_type==='machine') return '/machinery';
      return '/parts';
    }
    if(state.route==='seller') return routeHref('seller',{id:state.currentProduct||''});
    return routeHref(state.route||'home');
  }
  function updateSeo(){
    let title='Harvester Parts | Buy & Sell Farm Machinery, Tractor Parts and Harvester Spares';
    let description='Harvester Parts is a verified agricultural marketplace to buy and sell new or used farm machinery, OEM spare parts, tractor parts, combine harvester spares, implements and repair inventory across India.';
    let image=SITE_ORIGIN+'/logo-512.png';
    let type='website';
    const p=state.route==='product' ? findProductByRouteId(state.currentProduct) : null;
    if(p){
      title=`${p.title||'Agricultural Listing'} | Harvester Parts`;
      description=shortDescription(p.description,`Buy ${p.title||'agricultural machinery or spare part'} for ${money(p.price)}. Confirm condition, compatibility, location and delivery with the seller before ordering.`);
      image=absoluteMediaUrl(productImage(p));
      type='product';
    } else if(state.route==='market'){
      const cat=sessionStorage.hp_market_category||'';
      const brand=sessionStorage.hp_market_brand||'';
      const model=sessionStorage.hp_market_model||'';
      if(model&&brand){ title=`${brand} ${model} Parts and Machinery | Harvester Parts`; description=`Find ${brand} ${model} compatible agricultural spare parts and machinery listings from marketplace sellers in India.`; }
      else if(brand){ title=`${brand} Agricultural Parts and Machinery | Harvester Parts`; description=`Search ${brand} tractor, harvester and farm machinery parts listed by marketplace sellers in India.`; }
      else if(cat){ title=`${cat} in India | Harvester Parts`; description=`Buy and sell ${cat.toLowerCase()} with verified agricultural marketplace listings, seller profiles, location details and secure order records.`; }
      else { title='Buy Agricultural Spare Parts and Machinery in India | Harvester Parts'; description='Search by part name, OEM number, machine, model, brand, condition and location on Harvester Parts.'; }
    } else if(state.route==='categories'){
      title='Agricultural Machinery and Spare Part Categories | Harvester Parts';
      description='Browse combine harvester, tractor, rotavator, seed drill, straw reaper, bearing, belt, blade, hydraulic and other agricultural categories.';
    } else if(state.route==='request-part'){
      title='Request a Harvester or Tractor Spare Part | Harvester Parts';
      description='Submit a requirement for hard-to-find agricultural spare parts with machine type, brand, model, OEM number, photo and location.';
    } else if(legalDocs[state.route]){
      title=`${legalDocs[state.route].title} | Harvester Parts`;
      description=legalDocs[state.route].subtitle;
    } else if(state.route==='about'){
      title='About Harvester Parts | Indian Agricultural Marketplace';
    } else if(state.route==='sell'){
      title='Sell Agricultural Machinery and Spare Parts | Harvester Parts';
      description='Verified sellers can list new or used agricultural machinery, tractor parts, harvester spares and farm equipment with clear commission and payout tracking.';
    }
    const canonical=SITE_ORIGIN+currentCanonicalPath();
    ...
    setJsonLd('hp-jsonld-organization',{"@context":"https://schema.org","@type":"Organization","name":"Harvester Parts","url":SITE_ORIGIN,"logo":SITE_ORIGIN+"/logo-512.png","description":"Verified marketplace for agricultural machinery, combine harvester parts, tractor spares, farm implements and repair inventory in India.","contactPoint":{"@type":"ContactPoint","email":SUPPORT_EMAIL,"contactType":"customer support","areaServed":"IN"}});
    setJsonLd('hp-jsonld-website',{"@context":"https://schema.org","@type":"WebSite","name":"Harvester Parts","url":SITE_ORIGIN,"potentialAction":{"@type":"SearchAction","target":SITE_ORIGIN+"/parts?q={search_term_string}","query-input":"required name=search_term_string"}});
    const crumbs=[{"@type":"ListItem","position":1,"name":"Home","item":SITE_ORIGIN+"/"}];
    if(state.route==='market') crumbs.push({"@type":"ListItem","position":2,"name":"Marketplace","item":SITE_ORIGIN+"/parts"});
    if(p){
      crumbs.push({"@type":"ListItem","position":2,"name":p.category||'Marketplace',"item":SITE_ORIGIN+routeHref('market',{category:p.category||''})});
      crumbs.push({"@type":"ListItem","position":3,"name":p.title||'Product',"item":canonical});
      const ratings=reviewStats(p.user_id);
      const offer={"@type":"Offer","priceCurrency":"INR","price":Number(p.price||0),"availability":isVisibleProduct(p)?"https://schema.org/InStock":"https://schema.org/OutOfStock","url":canonical,"itemCondition":String(p.condition||'').toLowerCase()==='new'?"https://schema.org/NewCondition":"https://schema.org/UsedCondition","seller":{"@type":"Organization","name":sellerDisplayName(p)}};
      const productSchema={"@context":"https://schema.org","@type":"Product","name":p.title||'Agricultural listing',"image":productImages(p).filter(src=>!isVideoUrl(src)).map(absoluteMediaUrl),"description":description,"category":p.category||"Agricultural machinery and spare parts","brand":p.brand?{"@type":"Brand","name":p.brand}:undefined,"model":p.model||undefined,"sku":p.part_number||p.oem_number||detailValue(p.description,'OEM/part number')||String(p.id||''),"offers":offer};
      if(ratings.count) productSchema.aggregateRating={"@type":"AggregateRating","ratingValue":Number(ratings.average.toFixed(1)),"reviewCount":ratings.count};
      setJsonLd('hp-jsonld-dynamic',productSchema);
    } else {
      setJsonLd('hp-jsonld-dynamic',{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":crumbs});
    }
    setJsonLd('hp-jsonld-breadcrumbs',{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":crumbs});
  }
  ...
  function render(){ const [r,id]=parseRoute(); state.route=r||state.route||'home'; state.currentProduct=id||null; let html=''; if(state.route==='home')html=home(); else if(state.route==='market')html=market(); else if(state.route==='product')html=productPageProduction(state.currentProduct); else if(state.route==='seller')html=sellerProfilePage(state.currentProduct); else if(state.route==='cart')html=cartPage(); else if(state.route==='checkout')html=checkoutPage(); else if(state.route==='login')html=loginPage(); else if(state.route==='account')html=accountPageV113(); else if(state.route==='sell')html=sellPage(); else if(state.route==='messages')html=messagesPage(); else if(state.route==='orders')html=ordersPage(); else if(state.route==='admin')html=adminPageV113(); else if(state.route==='membership')html=membershipPage(); else if(state.route==='rewards')html=rewardsPage(); else if(state.route==='categories')html=categoriesPage(); else if(state.route==='about')html=aboutPage(); else if(state.route==='contact')html=contactPage(); else if(state.route==='how')html=howPage(); else if(state.route==='support')html=supportPage(); else if(state.route==='request-part')html=requestPartPage(); else if(state.route==='legal')html=legalCentrePage(); else if(legalDocs[state.route])html=legalDocPage(state.route); else html=home(); app.innerHTML=localizeHtml(html + legalFooter()); syncMenu(); bindPage(); applyLang(); updateSeo(); if(window.HP_APPLY_LANGUAGE) setTimeout(window.HP_APPLY_LANGUAGE,40); animateCounters(); startFactTicker(); setupPageMotion(); trackAnalyticsEvent('page_view',{route:state.route,product_id:state.currentProduct||''}); if(state.route==='orders')loadOrders(); if(state.route==='admin')loadAdminProData(); if(state.route==='seller')loadPublicProfile(state.currentProduct); }
  function bindPage(){
    ...
    $('#profileForm')?.addEventListener('submit',e=>{e.preventDefault();withLoading(e.target,()=>saveProfileV113(e.target),'Saving profile...')});
    $('#sellerReviewForm')?.addEventListener('submit',e=>{e.preventDefault();withLoading(e.target,()=>submitSellerReview(e.target),'Submitting rating...')});
    ...
    $('#partRequestForm')?.addEventListener('submit',e=>{e.preventDefault();withLoading(e.target,()=>submitPartRequest(e.target),'Sending request...')});
  ...
  function bindPayoutMethodChooser(){
    const form=$('#payoutAccountForm'); if(!form) return;
    const select=form.querySelector('select[name="payout_method"]');
    const sync=()=>{
      const method=select?.value || 'upi';
      const upi=form.querySelector('.payout-upi-fields');
      const bank=form.querySelector('.payout-bank-fields');
      upi?.classList.toggle('hidden-fields', method!=='upi');
      bank?.classList.toggle('hidden-fields', method!=='bank');
      upi?.querySelectorAll('input,select,textarea').forEach(el=>{ el.disabled=method!=='upi'; el.required=method==='upi'; });
      bank?.querySelectorAll('input,select,textarea').forEach(el=>{ el.disabled=method!=='bank'; el.required=method==='bank'; });
    };
  ...
  function syncSellerFormVisibility(form){
    ...
    const showSelf=mode==='self' && !!type;
    const showAssisted=mode==='assisted' && !!type;
    selfWrap?.classList.toggle('hidden-fields', !showSelf);
    assistedWrap?.classList.toggle('hidden-fields', !showAssisted);
    ...
    const selfTitle=form.querySelector('#selfListingTitle');
    const selfHint=form.querySelector('#selfListingHint');
    if(selfTitle) selfTitle.textContent=type==='spare'?'Self listing: spare part':type==='machine'?'Self listing: machinery':'Self listing';
    if(selfHint) selfHint.textContent=type==='spare'?'Use OEM/part number and table dimensions for exact matching.':'Add machine details, dimensions and delivery notes for heavy equipment.';
    const setDisabled=(wrap,disabled)=>wrap?.querySelectorAll('input,select,textarea,button').forEach(el=>{ el.disabled=disabled; });
    setDisabled(selfWrap, !showSelf);
    setDisabled(assistedWrap, !showAssisted);
    form.querySelectorAll('.self-listing-fields .machine-fields input,.self-listing-fields .machine-fields select,.self-listing-fields .machine-fields textarea').forEach(el=>{ el.disabled = !showSelf || type!=='machine'; });
    form.querySelectorAll('.self-listing-fields .spare-fields input,.self-listing-fields .spare-fields select,.self-listing-fields .spare-fields textarea').forEach(el=>{ el.disabled = !showSelf || type!=='spare'; });
    form.querySelectorAll('.spare-self-required').forEach(el=>{ el.required = mode==='self' && type==='spare'; });
    form.querySelectorAll('.assist-spare-required').forEach(el=>{ el.required = mode==='assisted' && type==='spare'; el.disabled = mode!=='assisted' || type!=='spare'; });
  }
  ...
  function filterMarket(){
    ...
    sessionStorage.hp_market_query=q;
    ...
    const brand=(sessionStorage.hp_market_brand||'').toLowerCase();
    const model=(sessionStorage.hp_market_model||'').toLowerCase();
    const forcedCondition=(sessionStorage.hp_market_condition||'').toLowerCase();
    const sort=$('#sortFilter')?.value||'all';
    let arr=visibleProducts().filter(p=>{
      const hay=[p.title,p.category,p.brand,p.model,p.condition,p.description,p.city,p.district,p.state,p.pincode,p.part_number,p.oem_number,p.sku_code,p.compatible_machine].join(' ').toLowerCase();
      return matchesProductType(p,type)
        && (!q||hay.includes(q))
        && (!cat||hay.includes(String(cat).toLowerCase()))
        && (!brand||hay.includes(brand))
        && (!model||hay.includes(model))
        && (!forcedCondition||String(p.condition||'').toLowerCase()===forcedCondition);
    });
