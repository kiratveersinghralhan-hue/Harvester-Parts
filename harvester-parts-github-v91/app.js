(() => {
  ...
  function t(k){return (i18n[state.lang]&&i18n[state.lang][k])||(i18n.en&&i18n.en[k])||k}
  ...
  function fallbackSlides(){
    return [
      {title:'Verified Farm Machinery',subtitle:'Tractors, harvesters, implements and used equipment from reviewed sellers.',cta_text:'Enter Marketplace',cta_route:'market',image_url:'./harvester-logo-full.jpg'},
      {title:'Fresh Spare Parts',subtitle:'Belts, bearings, blades, filters and workshop-ready stock for field repairs.',cta_text:'View Latest',cta_route:'market',image_url:'./harvester-logo-full.jpg'},
      {title:'Seller Plans and Payouts',subtitle:'Listing limits, platform fees, balances and payout requests are built into the marketplace.',cta_text:'Seller Plans',cta_route:'membership',image_url:'./harvester-logo-full.jpg'}
    ];
  }
  ...
  function home(){
    ...
    return `<section class="hero video-hero pro-home-hero"><video class="hero-bg-video" autoplay muted loop playsinline preload="metadata" poster="./harvester-logo-full.jpg"><source src="./hero-bg.mp4" type="video/mp4"></video><div class="hero-copy"><span class="eyebrow">Verified agricultural marketplace</span><h1>Farm machinery and spare parts marketplace.</h1><p>Buy or sell new and used agricultural machines, implements and parts with verified listings, in-app enquiries, checkout and seller payout controls.</p><div class="hero-actions"><button class="primary" data-route="market">Browse Marketplace</button><button class="secondary" data-route="sell">Start Selling</button><button class="ghost" data-route="membership">View Seller Plans</button></div><div class="trust-strip"><span>Admin-reviewed sellers</span><span>New and used listings</span><span>Buyer and seller commission flow</span></div></div><div class="hero-card glass marketplace-console"><div class="console-media"><img src="./harvester-logo-full.jpg" alt="Harvester Parts marketplace"></div><div class="console-panel"><span class="badge verified">Live marketplace</span><h2>Machines, parts and dealer stock in one place.</h2><p class="muted">Search by crop need, machine type, spare category, brand, model and location.</p></div><div class="stats"><div class="stat"><b data-count="${state.stats.products||state.products.length}">0</b><span>Live products</span></div><div class="stat"><b data-count="${categoryCount}">0</b><span>Categories</span></div><div class="stat"><b data-count="${sellerCount}">0</b><span>Verified sellers</span></div><div class="stat"><b data-count="${state.stats.orders||0}">0</b><span>Orders</span></div></div></div></section><section class="commerce-flow"><div><span>For buyers</span><b>Compare machinery and spare parts</b><p>Use search, category filters, cart, checkout and in-app chat before confirming the final deal.</p></div><div><span>For sellers</span><b>List after verification</b><p>Dealers, agencies and owners can publish new or used machines and parts after admin review.</p></div><div><span>For platform revenue</span><b>Commission and plans built in</b><p>Buyer protection fee, seller platform fee, membership plans, balances and payout requests are already part of the workflow.</p></div></section>${homeCarousel()}${agriFactCard()}<section><div class="section-head"><h2>Shop by farming need</h2><p class="muted">Quick paths for harvesters, tractors, implements, workshop parts and field-ready equipment.</p></div><div class="agri-category-grid compact">${cats.slice(0,8).map(categoryCard).join('')}</div></section><section><div class="section-head"><h2>Fresh in the agri mandi</h2><button class="ghost" data-route="market">View all</button></div><div class="grid">${featured.length?featured.map(productCard).join(''):empty('No live products yet.')}</div></section>`;
  }
  ...
  function productPage(id){
    ...
    return `<section class="product-page"><div class="gallery page-card product-gallery"><div class="main-product-photo" onclick="HP.openGallery('${esc(p.id)}',0)"><img src="${esc(imgs[0])}" onerror="this.src='${placeholder(p.category)}'" alt="${esc(p.title||'Product')}"><span class="photo-count">${imgs.length} photo${imgs.length>1?'s':''}</span></div>${imgs.length>1?`<div class="gallery-thumbs">${imgs.map((img,i)=>`<button onclick="HP.openGallery('${esc(p.id)}',${i})"><img src="${esc(img)}" onerror="this.src='${placeholder(p.category)}'"></button>`).join('')}</div>`:''}</div><aside class="detail-stack sticky-buy"><div class="page-card"><span class="badge verified">Verified Farm Listing</span><h1>${esc(p.title||'Agricultural Product')}</h1><p class="muted">${esc(p.category||'Spare Part')} • ${esc(p.brand||'Harvester Parts')} ${p.model?`• ${esc(p.model)}`:''}</p><div class="price">${money(p.price)}</div><p class="muted">Confirm final price, freight and availability before payment.</p><div class="seller-mini-card"><b>${esc(sellerName)}</b><span>${esc(loc)}</span></div><div class="actions"><button class="primary" onclick="HP.buyNow('${esc(p.id)}')">Buy Now</button><button class="secondary" onclick="HP.addToCart('${esc(p.id)}')">Add to Cart</button><button class="ghost" onclick="HP.toggleWishlist('${esc(p.id)}')">Wishlist</button><button class="ghost" onclick="HP.route('messages',{id:'${esc(p.id)}'})">Message Seller</button></div></div><div class="summary-card buyer-only-card"><h3>Buyer assurance</h3><div class="summary-row"><span>Verified seller</span><b>Seller verified</b></div><div class="summary-row"><span>Payment</span><b>Secure checkout</b></div><div class="summary-row"><span>Support</span><b>Support available</b></div></div></aside></section><section class="page-card"><h2>Product details</h2><p>${esc(p.description||'Agricultural machinery or spare part listing from a verified seller. Confirm compatibility, condition, dimensions and delivery details before final purchase.')}</p><div class="stats"><div class="stat"><b>${esc(p.condition||'Stock')}</b><span>Condition</span></div><div class="stat"><b>${esc(p.weight_kg||'—')} kg</b><span>Weight</span></div><div class="stat"><b>${esc(p.state||'India')}</b><span>Location</span></div><div class="stat"><b>${esc(p.views||0)}</b><span>Views</span></div></div></section><div id="photoLightbox" class="photo-lightbox" aria-hidden="true"><button class="lightbox-close" onclick="HP.closeGallery()">×</button><button class="lightbox-nav prev" onclick="HP.stepGallery(-1)">‹</button><img id="lightboxImage" src="${esc(imgs[0])}" alt="${esc(p.title||'Product photo')}"><button class="lightbox-nav next" onclick="HP.stepGallery(1)">›</button><div id="lightboxCount" class="lightbox-count">1 / ${imgs.length}</div></div>`;
  }
  ...
  function howPage(){
    return `<section class="page-card about-hero"><span class="eyebrow">How Harvester Parts works</span><h1>A verified marketplace built for agriculture trading.</h1><p>Buyers find machines and spare parts, sellers list after approval, and the platform keeps orders, messages, commission and payout records organized.</p></section><section class="about-grid process-grid"><div class="page-card process-card"><b>1</b><h2>Buyers browse or search</h2><p>Users can search by machine, spare part, brand, model, location and category.</p></div><div class="page-card process-card"><b>2</b><h2>Sellers get verified</h2><p>Farmers, dealers, agencies and machine owners submit seller details for admin review.</p></div><div class="page-card process-card"><b>3</b><h2>Listings go live</h2><p>Approved sellers publish new or used machinery and parts. Admin can remove unsafe or misleading products.</p></div><div class="page-card process-card"><b>4</b><h2>Checkout and chat stay inside</h2><p>Cart, checkout, messages, buyer fees, seller commission, balances and payout requests stay linked to the platform.</p></div></section>`;
  }
